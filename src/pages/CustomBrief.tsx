...
const CustomBriefContent = () => {
  ...
  useEffect(() => {
    document.title = "Custom Quote — SwiftLift";
  }, []);
    ...
    <main>
      ...
      <section className="relative overflow-hidden pt-28 pb-14 md:pt-36 md:pb-20 section-brand-dark">
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-6 text-center">
          ...
          >
            {lang === "en" ? "Premium Website Quote" : "高级网站报价"}
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            {lang === "en"
              ? "Tell us about your project and we'll prepare a tailored proposal within 48 hours."
              : "告诉我们您的项目需求，我们将在48小时内为您准备量身定制的方案。"}
          </motion.p>
        </div>
      </section>
          ...
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-10">
              ...
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-bold text-foreground font-display">
                    {lang === "en" ? "Budget & Timeline" : "预算与时间"}
                  </h3>
                  <span className="section-underline section-underline--light" />
                </div>

                <div>
                  <label className="text-sm font-medium text-foreground mb-2 block">{lang === "en" ? "Budget Range" : "预算范围"}</label>
                ...
                <div>
                  <label className="text-sm font-medium text-foreground mb-2 block">{lang === "en" ? "Preferred Timeline" : "期望时间"}</label>
                  <div className="grid grid-cols-2 gap-3">
